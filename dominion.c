...
void stateSupply(int count, struct gameState *state){
    state->supplyCount[estate] = count;
    state->supplyCount[duchy] = count;
...
int shuffle(int player, struct gameState *state) {
    ...
    int newDeck[MAX_DECK], newDeckPos = 0, card, i;
...
int scoreFor (int player, struct gameState *state) {
    ...
    for (i = 0; i < state->handCount[player]; i++)
    {
        return updateScore(player, state, score);
    }
...
int updateScore(int i, int player, struct gameState *state, int score) {
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
    ...
    switch( card )
    {
        ...
        case mine:
            return playMine(choice1, currentPlayer, choice2, state, handPos);
        ...
        case minion:
            return playMinion(currentPlayer, choice1, choice2, state, handPos);
        ...
        case ambassador:
            return playAmbassador(currentPlayer, choice1, choice2, state, handPos);
...
int playMine(int choice1, int currentPlayer, int choice2, struct gameState *state, int handPos) {
    int i, j;
    j = state->hand[currentPlayer][choice1];  //store card we will trash
